...
const FDDailyInterestGrid = ({
    ...
    const columns = useMemo(
        ...
        () => [
            ...
            {
                Header: "Interest Per Day",
                accessor: "InterestPerDay",
                width: 150,
                minWidth: 150,
            },
            {
    ...
    const downloadCSV = async () => {
        try {
            const url = `${BASE_URL}user/DownloadFDDailyInterestReport?FDNo=${searchData?.FDNo}&&FDAmt=${searchData?.FDAmt}&&FD_ROI=${searchData?.FD_ROI}&&FDInt_Type=${searchData?.FDInt_Type}&&FDInterestTerm=${searchData?.FDInterestTerm}&&FDStartDate=${searchData?.FDStartDate}&&FDMaturityDate=${searchData?.FDMaturityDate}`;
            // Create a temporary anchor element
            const link = document.createElement('a');
            link.href = url;
            // Set the download attribute with a default file name
            link.setAttribute('download', 'Fd-Daily-Interest-Report.xlsx');
...
export default FDDailyInterestGrid;
