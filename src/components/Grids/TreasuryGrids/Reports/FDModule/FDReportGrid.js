...
const FDReportGrid = ({
    ...
    const downloadCSV = async () => {
        try {
            const url = `${BASE_URL}user/DownloadFDReportData?BankName=${searchData?.Bank_Name}&&Status=${searchData?.Status}&&CompanyName=${searchData?.Co_Name}&&FDNo=${searchData?.FD_No}&&fromDate=${searchData?.From_Date === null ? '' : searchData?.From_Date}&&toDate=${searchData?.To_Date === null ? '' : searchData?.To_Date}`;
            // Create a temporary anchor element
            const link = document.createElement('a');
            link.href = url;
            // Set the download attribute with a default file name
            link.setAttribute('download', 'Fd-Report.xlsx');
