...
const BGCommissionReportGrid = ({
    ...
    const columns = useMemo(
        ...
        () => [
            ...
            {
                Header: "Days Utilized",
                accessor: `${'Days Utilized'}`,
                width: 150,
                minWidth: 150,
            },
    ...
    const downloadCSV = async () => {
        try {
            let url = `${BASE_URL}user/DownloadBGCommissionReportData?BankName=${searchData?.Bank_Name}&PDFCode=0&Status=${searchData?.Status}&CompanyName=${searchData?.Entity_Name}&BGNo=${searchData?.Bg_No}&BGRefNo=${searchData?.BG_Ref_No}&EffectiveDate=${searchData?.From_Date}&ExpiryDate=${searchData?.To_Date}`;

            // Create a temporary anchor element
            const link = document.createElement('a');
            link.href = url;
            // Set the download attribute with a default file name
            link.setAttribute('download', 'BG-Commission-Report.xlsx');
...
export default BGCommissionReportGrid;
