...
const PledgeWithMasterGrid = ({
  ...
  const downloadCSV = async () => {
    try {
      const url = `${BASE_URL}user/DownloadMFPledgeWithMaster?PledgeWithId=${0}&BankNameExchange=${searchInput}`;
      // Create a temporary anchor element
      const link = document.createElement('a');
      link.href = url;
      // Set the download attribute with a default file name
      link.setAttribute('download', 'PledgeWith.xlsx');
...
export default PledgeWithMasterGrid;
