...
const RoleGrid = ({
      ...
      },
      {
        ...
        Cell: ({ row }) => (
          <div className="text-center action-button">
            ...
            <DeleteButton onDelete={() => handleDeleteRole(row.original)} />
          </div>
        ),
  ...
  const data = useMemo(() => roleInfo || [], [roleInfo]);
  ...
  const downloadCSV = () => {
    ...
    if (link.download !== undefined) {
      ...
      link.setAttribute("download", "Role-Data.csv");
                ...
                <tr {...headerGroup.getHeaderGroupProps()}>
                  {headerGroup.headers.map((column) => (
                    ...
                    >
                      ...
                      {/* Render sorting arrows only if column is sortable */}
                      {!column.disableSortBy && (
                        <span
                          style={{
