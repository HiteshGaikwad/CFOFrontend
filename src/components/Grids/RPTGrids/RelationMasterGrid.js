...
const RelationMasterGrid = ({
  ...
  const downloadCSV = async () => {
    try {
      ...
      const url = `${BASE_URL}user/DownloadRPTRelationMaster?ReportStatus=0&RelationName=${searchInput ? searchInput : ''}`;
      // Create a temporary anchor element
      const link = document.createElement('a');
      link.href = url;
      // Set the download attribute with a default file name
      link.setAttribute('download', 'relation-master.xlsx');
...
export default RelationMasterGrid;
