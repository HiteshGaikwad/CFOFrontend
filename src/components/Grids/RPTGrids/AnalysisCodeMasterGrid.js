...
const AnalysisCodeMasterGrid = ({
  ...
  const data = useMemo(() => analysisCodeInfo || [], [analysisCodeInfo]);
  ...
  const downloadCSV = async () => {
    try {
      ...
      const url = `${BASE_URL}user/DownloadRPTAnalysiscodeMaster?ReportStatus=0&Description=${searchInput ? searchInput : ''}`;
      // Create a temporary anchor element
      const link = document.createElement('a');
      link.href = url;
      // Set the download attribute with a default file name
      link.setAttribute('download', 'analysis-code-master.xlsx');
...
export default AnalysisCodeMasterGrid;
